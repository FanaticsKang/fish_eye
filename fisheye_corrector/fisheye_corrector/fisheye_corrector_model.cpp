...
float FisheyeCorrector::distortion_list(float theta_in) {
  float theta_in_2 = theta_in * theta_in;
  float theta_in_4 = theta_in_2 * theta_in_2;
  float theta_in_6 = theta_in_4 * theta_in_2;
  return tan(theta_in /
             (1 + k1 * theta_in_2 + k2 * theta_in_4 + k3 * theta_in_6)) *
         f_camera_ * pixelHeight_;
}

void FisheyeCorrector::generateMap() {
  float angle_between_original_axis = radianToDegree(asin(
      sqrt(pow(sin(axis_vertical_radian_), 2) +
           pow(sin(axis_horizontal_radian_) * cos(axis_vertical_radian_), 2))));
  float radius_in_fisheye =
      distortion_list(degreeToRadian(angle_between_original_axis)) /
      pixelHeight_;
  print_value(radius_in_fisheye);
  ...
  float trans_x =
      cos(axis_vertical_radian_) * f_image_ * sin(axis_horizontal_radian_);
  float trans_z =
      -f_camera_ + f_image_ * cos(degreeToRadian(angle_between_original_axis));
  ...
  original_axis = Eigen::Vector3f(original_camera_plane_center - camera_center)
                      .normalized();
  Eigen::Vector3f object_axis =
      (new_camera_plane_center - camera_center).normalized();
  quaternion.setFromTwoVectors(original_axis, object_axis);
  quaternion.normalize();

  Eigen::Quaternion<float> quaternion_axis(cos(axis_rotation_radian_), 0, 0,
                                           sin(axis_rotation_radian_));
  quaternion_axis.normalize();
  Eigen::Matrix3f rotation =
      quaternion_axis.toRotationMatrix() * quaternion.toRotationMatrix();
  transform_camera_to_originalplane_ << rotation(0, 0), rotation(0, 1),
      rotation(0, 2), new_camera_plane_center(0), rotation(1, 0),
      rotation(1, 1), rotation(1, 2), new_camera_plane_center(1),
      rotation(2, 0), rotation(2, 1), rotation(2, 2),
      new_camera_plane_center(2), 0, 0, 0, 1;

  // std::cout << "transform_camera_to_originalplane_" << std::endl <<
  // transform_camera_to_originalplane_ << std::endl;
  for (int h = 0; h < Height_; h++)
    for (int w = 0; w < Width_; w++) {
      ...
      // float  radius_in_fisheye_floor =
      // distortion_list(degreeToRadian(degree));
      // float  radius_in_fisheye_ceil = distortion_list_[position_ceil];
      float radius_in_fisheye = distortion_list(degreeToRadian(
          degree));  //= radius_in_fisheye_floor + (radius_in_fisheye_ceil -
                     //radius_in_fisheye_floor)*((degree * 10 - position_floor)
...
FisheyeCorrector::FisheyeCorrector(std::string correction_table_file,
  ...
  std::cout << "input_width " << input_width << "input_height " << input_height
            << std::endl;
  size_scale_ = 1;
